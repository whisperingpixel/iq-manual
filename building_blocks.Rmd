...
Entities are always defined by means of one or more **properties**. For example, a property of water may be its blueish color, but for larger water bodies also its approximate flatness (i.e. a slope of approximately 0%). An entity definition block thus *must* be followed by one or more property definition blocks.
...
A result is the output of a single query. Depending on the processes that are used in the query, results can be single values, one-dimensional arrays or multi-dimensional cubes. Result definition blocks are exclusively used inside the **application** component of the model. Stored result definitions can be retrieved at any place within other result definitions inside the same model. *Not* every result definition needs to be exported. That is, a result definition can also be used as a preliminary outcome that serves only as a first step towards the final output of the query.

### Seasons and partitions {#partition}
...
Season and partition definition blocks are considered semantic concepts, and are therefore exlusively used inside the **semantic concept dictionary** component of the model. Unlike entities and results, the seasons and partition definitions are *not* evaluated at the moment they are defined. The temporal dimension of a cube is only split at the moment a specific season or partition is referenced inside the application component of the model.

## Data

Data blocks enable to access the source data of the semantic querying system. This is important when defining entities, but also, for example, when returning cloud-free mosaics. Several data sources can be accessed.

### Appearance

*<span style="color: red;"> Explain in one or two sentences </span>*

This data block refers to information about how a measurement looks based on some sort of interpretation of what it represents. This can be calculated indices (e.g. multi-spectral greenness indices, multi-spectral brightness values) or categories (e.g. multi-spectral color spaces/categories) with some level of semantic association. In the case of the sen2cube.at project, SIAM semantic enrichment generates color types with low-level semantic association that are accessible using these blocks.

### Atmosphere

*<span style="color: red;"> Explain in one or two sentences </span>*

This data block accesses all interpretations of measurements or additional information associated with atmospheric phenomena or influences of an image (e.g. not the Earth's surface). This includes estimates of haze, cloud cover, fog, airborne particulates, etc. that inhibit Earth surface applications, or are of specific interest to atmospheric applications.

### Reflectance

*<span style="color: red;"> Explain in one or two sentences </span>*

The calibrated reflectance values (e.g. top-of-atmosphere, bottom-of-atmosphere) of satellite imagery can be directly accessed using this data block on an individual band basis for a given sensor (i.e. calibrated pixel values in each captured band for a given sensor/platform).

### Topography

*<span style="color: red;"> Explain in one or two sentences </span>*

Any data or information related to the Earth's topography can be accessed using this data block (e.g. digital elevation models with derived aspect and slope).

### Artifacts

*<span style="color: red;"> Explain in one or two sentences </span>*

This data block includes all measurements or interpretations of measurements that may be associated with imagery artifacts (e.g. perceived/detected measurement errors, differences in quality across an image, edges of acquisition swaths).

### Metadata

Metadata refers to the metadata of the satellite images from which the data were taken. For example, you can replace all observed values in a cube by the timestamps at which those observations were made.
...
sen2cube has several built-in operators that can be used to build expressions.
...
Similarly, we can also reduce the spatial dimensions instead, ending up with a single value per timestamp. The sen2cube interface only supports reduction of the two spatial dimensions at once, and *not* of either the X or Y dimension separately.
...
sen2cube has several built-in reduction functions that can be used.
...
*<span style="color: red;"> Insert toy examples for all reduction functions </span>*
