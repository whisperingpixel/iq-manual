...
The Sen2Cube.at semantic image querying system is accessible through a [web-interface](https://demo.sen2cube.at/app), in which queries can be formulated by intuitively combining several building blocks, that each represent a distinct, clearly defined task or value inside the semantic querying process. This manual contains the reference documentation of that interface.
...
The continuous observation with acquisition frequencies of a few days allow to extract relevant patterns based on (long) time series of a selected variable. For example, by using the appearance of "greenness" of the observatons as variable, it is possible to trace back the land use change of this area where an industrial/commercial building was constructed on a meadow.
...
```{block, type='rmdnote'}
The Sen2Cube.at system was not explicitly designed for any of these use-cases. All of them were created using the same, generic semantic enrichment and generic building blocks of the semantic query language to construct the different queries for each example. Still, it is not necessary to do any programming - you don't have to trade-of between the need of programming or flexibility of the software.
```

Make yourself familiar with the [Semantic Enrichment](#enrichment) of Earth observation data and then proceed with the [Getting Started](#start) section.