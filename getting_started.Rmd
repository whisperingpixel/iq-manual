...
Sen2Cube.at is a Web-application that can be accessed with a standard browser. Installing additional software on your local computer is not necessary. The Web-frontend of our semantic querying interface is available online at [demo.sen2cube.at](https://demo.sen2cube.at/app). 

## Components

Inferring information from data using an expert system (information processing system) is typically achieved by combining a **knowledgebase** and a **factbase** that allows the **inference engine** to process large volumes of data automatically. The knowledgebase contains general or specific knowledge of the world, expressed in rulesets and is augmented by users incrementally through using the system. The factbase contains the raw image data as well as the semantically enriched data.
...
We recommend that you download important results as they will be deleted. However, we will keep the inference settings, which allows you to re-process them at any time.
```

In the case the query could *not* be processed successfully, an error message describing the problem will appear instead.

*<span style="color: red;"> Insert video that showcases the full workflow </span>*